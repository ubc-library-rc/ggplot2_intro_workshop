...
Using the countries dataset, plot gdpPercap (gdp per capita, x variable) vs lifeExp (life expectancy). Have each year in the dataset be a unique colour, and set the colours to the viridis scale. Add a linear trendline to the plot.
...
```{r}
countries$year <-
  as.integer(countries$year) #need our data to be numeric instead of categorical for this
...
plot07 <-
  ggplot(aes(x = log(gdpPercap), y = lifeExp), data = countries07) +
