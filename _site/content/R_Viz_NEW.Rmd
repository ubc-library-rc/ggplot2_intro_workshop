...
ggplot(aes(x = Petal.Length, y = Sepal.Length), data = iris) +
  ...
  geom_smooth(method = 'lm', se = FALSE, col = 'black') #we     can also specify the color of the trendline 
```
...
When you specify OUTSIDE aes(), it is a simple aesthetic specification and you provide the colour directly (in this case, 'black'). 
...
ggplot(aes(x = Petal.Length, y = Sepal.Length), data = iris) +
  ...
  scale_color_manual(values = c("turquoise3", "goldenrod1","salmon2")) 
```
...
ggplot(aes(x=log(gdpPercap), y=lifeExp), data = countries)+
  geom_point(aes(col = year)) +
  geom_smooth(aes(col = year),method = 'lm', se = FALSE) +
...
We could also make our plot interactive, instead of animated. For illustrative purposes, we will restrict our data to 2007. 

```{r}

countries07 <- countries[countries$year==2007,]
